...
os.makedirs(non_flaky_methods_dir, exist_ok=True)

# Load quantum non-flakiness dataset
dataset_path = 'NoneFlacky_urls.csv'
dataset = pd.read_csv(dataset_path)

# Function to extract repo name and PR number from the PR URL
def extract_repo_and_pr(url):
    parts = url.split('/')
    repo_name = '/'.join(parts[3:5])  # This gets the 'owner/repo' part
    pr_number = parts[-1]  # This gets the PR number
...
# Function to extract non-flaky methods from the base commit and print their URLs
def extract_non_flaky_methods(dataset, non_flaky_methods_dir):
    errors = []
    added_files = []
    records = []  # List to store records for the Excel file
    for index, row in dataset.iterrows():
        pr_url = row['0']  # Updated to use the correct column name
        if not pd.isna(pr_url):
            print(f"Processing {pr_url}...")
            repo_name, pr_number = extract_repo_and_pr(pr_url)
            try:
                ...
                for file in files:
                    if file['filename'].endswith('.py'):
                        ...
                        try:
                            fetch_and_save_file(repo_name, pr_number, base_commit_sha, file_path, raw_url, non_flaky_methods_dir)
...
# Extract non-flaky methods and collect data for Excel
records = extract_non_flaky_methods(dataset, non_flaky_methods_dir)

# Step 2: Create an Excel file from the collected data
df = pd.DataFrame(records)
output_excel_path = os.path.join(output_dir, 'repo_non-flaky.xlsx')
